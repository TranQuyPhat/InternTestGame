...
game.keydown = function (event) {
  if (!game.pressedKeys[event.keyCode]) {
    ...
    switch (event.keyCode) {
      ...
      case 32:
        // Check for player input or conditions for jump/double jump
        if (!game.player.isInAir) countJump = 0;

        countJump++;

        if (!game.player.isInAir && countJump == 1) {
          game.player.jump("", countJump);
        } else if (
